...
extern "C" {            // Prevents name mangling of functions
    ...
    RAYGUIAPI int GuiIsLocked(void);                               // Check if gui is locked (global state)
    ...
    RAYGUIAPI char **GuiLoadIcons(const char *fileName, int loadIconsName); // Load raygui icons file (.rgi) into internal icons data
    ...
    RAYGUIAPI int GuiToggle(Rectangle bounds, const char *text, int *active);                             // Toggle Button control
    ...
    RAYGUIAPI int GuiCheckBox(Rectangle bounds, const char *text, int *checked);                          // Check Box control, returns true when active
    RAYGUIAPI int GuiComboBox(Rectangle bounds, const char *text, int *active);                            // Combo Box control
    
    RAYGUIAPI int GuiDropdownBox(Rectangle bounds, const char *text, int *active, int editMode);          // Dropdown Box control
    RAYGUIAPI int GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, int editMode); // Spinner control
    RAYGUIAPI int GuiValueBox(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, int editMode); // Value Box control, updates input text with numbers
    RAYGUIAPI int GuiValueBoxFloat(Rectangle bounds, const char* text, char *textValue, float *value, int editMode);       // Value box control for float values
    RAYGUIAPI int GuiTextBox(Rectangle bounds, char *text, int textSize, int editMode);                   // Text Box control, updates input text
...
static int guiLocked = false;                  // Gui lock state (no inputs processed)
...
static int guiTooltip = false;                 // Tooltip enabled/disabled
static const char *guiTooltipPtr = NULL;        // Tooltip string pointer (string provided by user)

static int guiControlExclusiveMode = false;    // Gui control exclusive mode (no inputs processed except current control)
...
static int guiStyleLoaded = false;         // Style loaded flag for lazy style initialization
...
static int IsMouseButtonReleased(int button);
...
static int CheckCollisionPointRec(Vector2 point, Rectangle rec);   // Check if point is inside rectangle
...
int GuiIsLocked(void) { return guiLocked; }
...
#define RAYGUI_TABBAR_ITEM_WIDTH    160
    ...
    int toggle = false;    // Required for individual toggles
...
#define RAYGUI_MIN_MOUSE_WHEEL_SPEED   20
    ...
    int hasHorizontalScrollBar = (content.width > bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH))? true : false;
    int hasVerticalScrollBar = (content.height > bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH))? true : false;
...
int GuiToggle(Rectangle bounds, const char *text, int *active)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiCheckBox(Rectangle bounds, const char *text, int *checked)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiDropdownBox(Rectangle bounds, const char *text, int *active, int editMode)
{
    int result = 0;
    GuiState state = guiState;
...
#define RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY      1        // Frames delay for autocursor movement
    ...
    int multiline = false;     // TODO: Consider multiline text input
...
int GuiTextBoxMulti(Rectangle bounds, char *text, int textSize, int editMode)
{
    int pressed = false;
...
int GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, int editMode)
{
    int result = 1;
    GuiState state = guiState;
...
int GuiValueBox(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, int editMode)
{
...
// NOTE: Requires static variables: frameCounter
int GuiValueBoxFloat(Rectangle bounds, const char *text, char *textValue, float *value, int editMode)
{
#if !defined(RAYGUI_VALUEBOX_MAX_CHARS)
#define RAYGUI_VALUEBOX_MAX_CHARS  32
#endif
...
char **GuiLoadIcons(const char *fileName, int loadIconsName)
{
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    int isVertical = (bounds.width > bounds.height)? false : true;
...
static int CheckCollisionPointRec(Vector2 point, Rectangle rec)
{
    int collision = false;
