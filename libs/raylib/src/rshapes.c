...
void DrawRectangleSmooth(Vector2 startPoint, Vector2 endPoint, float size, Color color)
{
    Vector2 topLeft = { 0 };
    Vector2 topRight = { 0 };
    Vector2 bottomLeft = { 0 };
    ...
    Vector2 tangent = {endPoint.x - startPoint.x, endPoint.y - startPoint.y};
    float length = 0.5f*size/sqrt(tangent.x*tangent.x + tangent.y*tangent.y);
    Vector2 normal = {-tangent.y * length, tangent.x*length};
    topLeft = (Vector2){-normal.x + startPoint.x, -normal.y + startPoint.y};
    topRight = (Vector2){-normal.x + endPoint.x, -normal.y + endPoint.y};
    bottomLeft = (Vector2){normal.x + startPoint.x, normal.y + startPoint.y};
    bottomRight = (Vector2){normal.x + endPoint.x, normal.y + endPoint.y};
    ...
    rlTexCoord2f(1, texShapesRec.y/texShapes.height);
    rlVertex2f(topLeft.x, topLeft.y);
    
    rlTexCoord2f(-1, (texShapesRec.y + texShapesRec.height)/texShapes.height);
    rlVertex2f(bottomLeft.x, bottomLeft.y);
...
void DrawPolygonGradient(Vector2 vertex1, Vector2 vertex2, Vector2 vertex3, Vector2 vertex4, float t0, float t1)
{
    rlSetTexture(texShapes.id);
    ...
    rlTexCoord2f(t1, 0.5f);
    rlVertex2f(vertex1.x, vertex1.y);
    
    rlTexCoord2f(t1, 0.5f);
    rlVertex2f(vertex2.x, vertex2.y);
    
    rlTexCoord2f(t0, 0.5f);
    rlVertex2f(vertex4.x, vertex4.y);
    
    rlTexCoord2f(t0, 0.5f);
    rlVertex2f(vertex3.x, vertex3.y);
...
void DrawCircleSmooth(Vector2 pos, float radius, Color color){
    Vector2 topLeft = { 0 };
    Vector2 topRight = { 0 };
    Vector2 bottomLeft = { 0 };
    Vector2 bottomRight = { 0 };
    
    topLeft = (Vector2){pos.x-radius, pos.y-radius};
    topRight = (Vector2){pos.x+radius, pos.y-radius};
    bottomLeft = (Vector2){pos.x-radius, pos.y+radius};
    bottomRight = (Vector2){pos.x+radius, pos.y+radius};
